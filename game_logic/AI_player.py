...
class AIPlayer(player.Player):
    ...
    def get_move(self, board):
        if board.check_empty():
            best_move_list = [0, 2, 4, 6, 8]
            best_move = choice(best_move_list)
        else:
    ...
    def calculate_score(self, board):
        ...
        score = 0
        if board.board_list[move] == self.marker:
            ...
            for line in board.win_combinations:
                if move in line:
                    line_score = 0
                    board_line = [board.board_list[i] for i in line]
                    if board_line.count(self.opp_player()) == 3:
                        line_score = -10
    ...
    def calculate_score_dumb(self, board):
        if board.check_win_for_player(self):
            return 1
        elif board.check_win_for_player(player.Player(self.opp_player())):
            return -1
    ...
    def maximize(self, board, depth):
        ...
        for possible_move in board.get_possible_moves():
            board.apply_move(possible_move)
            board.turn_player()
